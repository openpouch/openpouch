...
describe("openpouch-run adapter", () => {
  ...
  it("getService and getRecentDeploys map a deployment DTO", async () => {
    ...
    // A-6: the live expiry survives the mapping — `inspect` is the resume
    // surface where an agent asks "how long does this app still live?".
    expect(deploys[0]!.expiresAt).toBe("2026-06-15T00:00:00.000Z");
  });
