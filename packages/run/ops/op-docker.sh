...
VOLUMES_DIR="${OP_VOLUMES_DIR:-/var/openpouch/volumes}"
...
# Volume key = "<accountId>/<project>" — exactly one slash, both segments from
# server-controlled alphabets (acct ids are `acct_<hex>`, projects are app
# names). No dots, so no traversal; the path is still realpath-checked below.
valid_volkey() { [[ "$1" =~ ^[a-z0-9_]{1,64}/[a-z0-9][a-z0-9-]{0,39}$ ]] || die "invalid volume key: $1"; }
...
# Derive the /data bind-mount source from a validated volume KEY (L13) — like
# site_dir_for, run-d never supplies a path, only the key; the wrapper owns the
# base. The dir must already exist (run-d's ensureVolumeDir creates it) and is
# canonicalized so a symlink cannot escape VOLUMES_DIR.
volume_dir_for() {
  local key="$1" dir real base
  dir="$VOLUMES_DIR/$key"
  [ -d "$dir" ] || die "volume dir not found: $dir"
  real="$(realpath "$dir")" || die "cannot resolve volume dir"
  base="$(realpath "$VOLUMES_DIR")" || die "cannot resolve volumes dir"
  case "$real/" in "$base"/*) : ;; *) die "volume dir escapes $base" ;; esac
  printf '%s' "$real"
}
...
case "$verb" in
  ...
  buildapp)  # buildapp <slug> <buildCmd> — run the app's build script (e.g. npm run build)
    ...
    exec "${BUILD_CMD[@]}"
    ;;
  create)  # create <slug> <hostPort> <startCmd> [envB64] [volKey]   envB64 = "KEY base64(value)" lines · volKey = "<acct>/<project>" (L13; env may be "" when only a volume is passed)
    { [ "$#" -ge 3 ] && [ "$#" -le 5 ]; } || die "usage: create <slug> <hostPort> <startCmd> [env] [volKey]"
    valid_slug "$1"; valid_port "$2"; slug="$1"; port="$2"; start="$3"; site="$(site_dir_for "$slug")"
    # Optional persistent volume (5th arg): validated key → wrapper-derived path
    # bind-mounted read-write at /data (the only writable mount besides /app).
    VOL_ARGS=()
    if [ "$#" -eq 5 ] && [ -n "$5" ]; then
      valid_volkey "$5"
      vol="$(volume_dir_for "$5")"
      VOL_ARGS=(-v "$vol:/data")
    fi
