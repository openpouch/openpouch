...
describe("orchestrator — deploy", () => {
  ...
  it("splits captured output on lone \\r so progress frames become clean lines (Codex OP-005, 2026-07-13)", async () => {
    const { store, engine, orch } = await setup();
    engine.buildLogs = "transforming (1)\rtransforming (9)\r✓ 17 modules transformed.";
    const rec = await deploy(store, orch, "api", 0);
    const messages = store.get(rec.id)!.events.map((e) => e.message);
    expect(messages).toContain("[install] transforming (1)");
    expect(messages).toContain("[install] ✓ 17 modules transformed.");
    for (const m of messages) expect(m).not.toContain("\r");
  });
...
describe("L10 volumes + always-on (staged 0.3.0)", () => {
  it("mounts the account+project volume dir on create and keys it to survive redeploys", async () => {
    const { store, engine, orch } = await setup();
    const rec = await store.create({ bytes: 1, fileCount: 1, now: 0, kind: "dynamic", accountId: "acct_v", volumeProject: "shop" });
    await orch.deployDynamic(rec, { kind: "dynamic", reason: "test", startCmd: "node s.js", hasLockfile: false }, store.siteDir(rec.slug));
    const spec = engine.createSpecs.get(rec.slug)!;
    expect(spec.volumeDir).toBe(join(store.dataDir, "volumes", "acct_v", "shop"));
    // The wrapper-safe relative key rides along (L13 box fix 2026-07-18).
    expect(spec.volumeKey).toBe("acct_v/shop");
    // A REDEPLOY (new slug, same account+project) resolves to the SAME dir.
    const rec2 = await store.create({ bytes: 1, fileCount: 1, now: 1, kind: "dynamic", accountId: "acct_v", volumeProject: "shop" });
    await orch.deployDynamic(rec2, { kind: "dynamic", reason: "test", startCmd: "node s.js", hasLockfile: false }, store.siteDir(rec2.slug));
    expect(engine.createSpecs.get(rec2.slug)!.volumeDir).toBe(spec.volumeDir);
    // No volume requested → no /data mount, no key.
    const plain = await store.create({ bytes: 1, fileCount: 1, now: 2, kind: "dynamic", accountId: "acct_v" });
    await orch.deployDynamic(plain, { kind: "dynamic", reason: "test", startCmd: "node s.js", hasLockfile: false }, store.siteDir(plain.slug));
    expect(engine.createSpecs.get(plain.slug)!.volumeDir).toBeUndefined();
    expect(engine.createSpecs.get(plain.slug)!.volumeKey).toBeUndefined();
  });

  it("idleSweep never stops an always-on app; a plain app still idles out", async () => {
    const { store, engine, orch } = await setup();
    const keep = await store.create({ bytes: 1, fileCount: 1, now: 0, kind: "dynamic", accountId: "a", alwaysOn: true });
    const idle = await store.create({ bytes: 1, fileCount: 1, now: 0, kind: "dynamic", accountId: "a" });
    for (const r of [keep, idle]) {
      await orch.deployDynamic(r, { kind: "dynamic", reason: "t", startCmd: "node s.js", hasLockfile: false }, store.siteDir(r.slug));
    }
    const stopped = await orch.idleSweep(10 * 60_000); // far past idleMs=1000
    expect(stopped).toEqual([idle.slug]);
    expect(store.get(keep.slug)!.runtimeStatus).toBe("running");
    expect(engine.states.get(store.get(keep.slug)!.containerId!)).toBe("running");
  });
});
