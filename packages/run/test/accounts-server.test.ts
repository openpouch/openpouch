...
import type { Mailer, VerificationMail } from "../src/mailer.js";
...
describe("accounts: per-account quota", () => {
  it("enforces the tier's live-deployment cap with 429", async () => {
    const tiers: TierTable = {
      anonymous: { name: "anonymous", deploysPerHour: 100, deploysPerDay: 100, maxLiveDeployments: 1, maxBytes: 1e9, maxDynamic: 5 },
      free: { name: "free", deploysPerHour: 100, deploysPerDay: 100, maxLiveDeployments: 2, maxBytes: 1e9, maxDynamic: 5 },
    };
    const mailer = new CapturingMailer();
