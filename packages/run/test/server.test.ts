...
describe("run-d hardening", () => {
  ...
  it("event log says accepted at create and live only once servable (Codex OP-002, 2026-07-13)", async () => {
    const { base, store } = await start({});
    const tb = await makeTarball();
    expect(await deploy(base, tb)).toBe(201);
    const events = store.list()[0]!.events.map((e) => e.message);
    const accepted = events.indexOf("status: accepted");
    const live = events.indexOf("status: live");
    expect(accepted).toBeGreaterThanOrEqual(0);
    expect(live).toBeGreaterThan(accepted); // never "live" before the work above succeeded
  });
