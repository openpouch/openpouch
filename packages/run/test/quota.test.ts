...
describe("quota: tiersFromEnv (configurable, D16)", () => {
  ...
  it("fully replaces the table with a valid JSON override", () => {
    const custom = {
      anonymous: { deploysPerHour: 1, deploysPerDay: 2, maxLiveDeployments: 1, maxBytes: 100, maxDynamic: 0 },
      free: { deploysPerHour: 9, deploysPerDay: 9, maxLiveDeployments: 9, maxBytes: 999, maxDynamic: 9 },
    };
  ...
  it.each([
    ...
    ["missing anonymous", JSON.stringify({ free: { deploysPerHour: 1, deploysPerDay: 1, maxLiveDeployments: 1, maxBytes: 1, maxDynamic: 1 } })],
