...
export interface OrchestratorDeps {
   ...
   */
  clock?: () => number;
  /** M-1: called once per scale-to-zero wake (aggregate counter; never per-user data). */
  onWake?: () => void;
}
...
export class Orchestrator {
  ...
  async deployDynamic(record: DeploymentRecord, plan: DynamicPlan, siteDir: string, env?: Record<string, string>): Promise<void> {
    ...
    try {
      ...
      const volumeDir = await this.ensureVolumeDir(record);
      containerId = await this.engine.create({
        slug,
        siteDir,
        startCmd: plan.startCmd,
        hostPort: port,
        ...(env ? { env } : {}),
        ...(volumeDir ? { volumeDir, volumeKey: `${record.accountId}/${record.volumeProject}` } : {}),
      });
  ...
  /**
   * L12 (app updates): create+start+health-check a container for `slug` from its
   * CURRENT site dir — no store writes, no route sync; the caller owns record
   * truth and batches the sync. On any failure the container/port are cleaned
   * up here and a DeployFailedError (with logs) is thrown.
   */
  async startFreshContainer(
    slug: string,
    opts: { startCmd: string; env?: Record<string, string>; volumeRecord: DeploymentRecord },
  ): Promise<{ containerId: string; hostPort: number; readyMs: number }> {
    const port = this.ports.allocate();
    let containerId: string | undefined;
    const startAt = this.clock();
    try {
      const volumeDir = await this.ensureVolumeDir(opts.volumeRecord);
      containerId = await this.engine.create({
        slug,
        siteDir: this.store.siteDir(slug),
        startCmd: opts.startCmd,
        hostPort: port,
        ...(opts.env ? { env: opts.env } : {}),
        ...(volumeDir
          ? { volumeDir, volumeKey: `${opts.volumeRecord.accountId}/${opts.volumeRecord.volumeProject}` }
          : {}),
      });
      await this.engine.start(containerId);
      if (!(await this.waitHealthy(port))) {
        const logs = await this.engine.logs(containerId).catch(() => "");
        throw new DeployFailedError("app did not answer HTTP in time", logs);
      }
      this.lastSeen.set(slug, this.clock());
      return { containerId, hostPort: port, readyMs: this.clock() - startAt };
    } catch (err) {
      if (containerId) await this.engine.remove(containerId).catch(() => {});
      this.ports.release(port);
      throw err instanceof DeployFailedError ? err : new DeployFailedError((err as Error).message, "");
    }
  }

  /** L12: tear down one record's container/port (no route sync — callers batch it). */
  async releaseContainer(record: DeploymentRecord): Promise<void> {
    if (record.containerId) await this.engine.remove(record.containerId).catch(() => {});
    if (record.hostPort !== undefined) this.ports.release(record.hostPort);
    this.lastSeen.delete(record.slug);
  }
  ...
  async idleSweep(now: number): Promise<string[]> {
    ...
    for (const rec of this.store.list()) {
      ...
      if (rec.alwaysOn === true) continue; // paid always-on: never idle-stopped (staged 0.3.0)
  ...
  /**
   * L10 (staged 0.3.0): resolve + create the record's persistent volume dir —
   * `<dataDir>/volumes/<accountId>/<project>` — keyed to account+project so it
   * survives redeploys (new slug, same project), scale-to-zero, and reboots.
   * Returns undefined for records without a volume (the common case).
   */
  /** L13: stop a running dynamic container so its /data volume can be written
   *  consistently (SQLite etc.) — the caller restarts via ensureRunning. */
  async stopContainer(record: DeploymentRecord, now: number): Promise<void> {
    if (record.kind !== "dynamic" || !record.containerId || record.runtimeStatus !== "running") return;
    await this.engine.stop(record.containerId);
    await this.store.update(record.slug, { runtimeStatus: "stopped" });
    await this.store.appendEvent(record.slug, "stopped (data write)", now);
  }

  async ensureVolumeDir(record: DeploymentRecord): Promise<string | undefined> {
    if (!record.volumeProject || !record.accountId) return undefined;
    const dir = join(this.store.dataDir, "volumes", record.accountId, record.volumeProject);
  ...
  async reconcile(now: number): Promise<void> {
    for (const rec of this.store.list()) {
      ...
      if (state === "running") {
        ...
        if (rec.startCmd && rec.hostPort !== undefined) {
          try {
            const volumeDir = await this.ensureVolumeDir(rec);
            const id = await this.engine.create({ slug: rec.slug, siteDir: this.store.siteDir(rec.slug), startCmd: rec.startCmd, hostPort: rec.hostPort, ...(volumeDir ? { volumeDir } : {}) });
    ...
    // Staged 0.3.0: always-on apps come back up proactively after a reboot —
    // that is the promise the flag sells (no first-visitor cold start).
    for (const rec of this.store.list()) {
      if (rec.alwaysOn !== true || rec.kind !== "dynamic" || rec.runtimeStatus !== "stopped" || !rec.containerId) continue;
      try {
        await this.ensureRunning(rec, now);
      } catch (e) {
        this.log(`always-on restart failed for ${rec.slug}: ${(e as Error).message}`);
      }
    }
    await this.syncRoutes();
  ...
  private async recordOutput(slug: string, phase: "install" | "build" | "app", raw: string, now: number): Promise<void> {
    // Split on lone \r too: build tools (vite, npm) redraw progress lines with
    // bare carriage returns; leaving them inside a line puts C0 control chars
    // into `logs --json` messages (Codex OP-005, 2026-07-13).
