...
export class WrapperEngine implements ContainerEngine {
  ...
  async create(spec: CreateSpec): Promise<string> {
    ...
    const envLines = Object.entries(spec.env ?? {})
      ...
      .join("\n");
    // Optional persistent volume as the 5th POSITIONAL arg (L13): pass the
    // relative "<acct>/<project>" key, never a host path — the wrapper derives
    // and realpath-checks the mount source itself. When a volume rides along
    // with no env, the env slot is filled with "" so positions stay stable.
    const volumeKey = spec.volumeKey ?? "";
    if (envLines !== "" || volumeKey !== "") args.push(envLines);
    if (volumeKey !== "") args.push(volumeKey);
