...
export interface CreateSpec {
  ...
  /** L10 (staged 0.3.0): host dir bind-mounted read-write to /data — persistent
   *  storage keyed to account+project, surviving redeploys and reboots. Used by
   *  the raw DockerEngine, which may mount arbitrary host paths. */
  volumeDir?: string;
  /** L13 box-rollout fix (2026-07-18): the same volume as a RELATIVE
   *  "<accountId>/<project>" key for the privileged op-docker wrapper, which
   *  never accepts host paths from run-d — it derives and realpath-checks the
   *  mount source from this validated key itself (site_dir_for pattern). The
   *  raw DockerEngine ignores it. Always set together with volumeDir. */
  volumeKey?: string;
}
...
export class DockerEngine implements ContainerEngine {
  ...
  async create(spec: CreateSpec): Promise<string> {
    ...
    const args = [
      ...
      ...(spec.volumeDir ? ["-v", `${spec.volumeDir}:/data`] : []),
