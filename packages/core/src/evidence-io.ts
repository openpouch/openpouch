...
export function renderDeploymentMd(evidence: Evidence): string {
  ...
  if (latestByEnv.size === 0) {
    ...
    // breakage (OpenClaw 2026-07-05 #3). Commands use the `npx -y` form: this
    // file is read by a FRESH session that may have no global `openpouch`
    // binary — a bare command would exit 127 there (Codex F-02, 2026-07-12);
    // npx also resolves a locally/globally installed copy first.
    const resumeHealthPath = evidence.deployments.find(
      (d) => d.healthPath !== undefined && d.healthPath !== "/",
    )?.healthPath;
    const verifyCmd = resumeHealthPath !== undefined ? `npx -y openpouch verify --json --health-path ${resumeHealthPath}` : "npx -y openpouch verify --json";
    lines.push("## Resume after context loss", "");
    lines.push("An agent returning to this project re-establishes state with:", "");
    lines.push("- `npx -y openpouch inspect --json` — current deployment status (environment, URL, provider)");
    lines.push(
      resumeHealthPath !== undefined
        ...
        : "- `npx -y openpouch verify --json` — health-check the live URL (GET / expects 200)",
    );
    lines.push("- `npx -y openpouch logs --json` — recent install/build/app output (shows why a deploy is unhealthy)");
    lines.push("");
    lines.push(
    ...
    lines.push("1. `npx -y openpouch logs --limit 200 --json` — find the failing phase (lines are tagged `[install]` / `[build]` / `[app]`).");
    ...
    lines.push(
      resumeHealthPath !== undefined
        ...
        : "4. `npx -y openpouch verify --json` — for a full-stack app add `--health-path /api/...` so the API is proven too, not just `/`.",
    );
