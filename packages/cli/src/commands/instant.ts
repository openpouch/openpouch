...
import { dataDurabilityNote, humanList, instantDeploySummary, tellYourUser } from "../summary.js";
 ...
 */
export function buildTarball(cwd: string, extraExcludes: string[] = []): Promise<Buffer> {
...
export async function instantCommand(
  ctx: CliContext,
  dir?: string,
  envOpts?: { envFile?: string; vars?: string[]; redactSecrets?: boolean; healthPath?: string; volume?: boolean; alwaysOn?: boolean; app?: string },
): Promise<CommandResult> {
  // L12 `--app <name>`: a named app keeps ONE stable URL across redeploys —
  // the first deploy mints it, every later one updates behind it. Validate the
  // name up front (DNS-label-like, mirrors the server rule).
  const appName = envOpts?.app;
  if (appName !== undefined && !/^[a-z0-9](?:[a-z0-9-]{0,38}[a-z0-9])?$/.test(appName)) {
    return errorResult(
      EXIT.USAGE,
      "usage",
      `invalid --app name "${appName}" — use 1-40 chars of a-z, 0-9 and inner dashes`,
      "Pick a DNS-label-like name, e.g. `--app my-shop`.",
    );
  }
  ...
  let migratingFrom: string[] = [];
  if (loaded.status === "ok") {
    const envs = Object.values(loaded.manifest.environments ?? {});
    const governedProviders = [...new Set(envs.map((e) => e.provider).filter((p) => p !== "openpouch-run"))];
    if (governedProviders.length > 0) {
      // L13/G3 — the intent fork lives in the MOMENT OF ACTION, not only in the
      // docs (field report 2026-07-16: an agent sold governance as a migration).
      // With --app this IS the migration move: deploy ONTO openpouch hosting
      // while the old host keeps running untouched as the rollback anchor.
      if (appName === undefined) {
        return errorResult(
          EXIT.USAGE,
          "usage",
          "this project already has deploy.manifest.json (a governed " + governedProviders.join("/") + " setup)",
          "Use `openpouch preview` / `openpouch prod` to deploy THERE. To MIGRATE the app onto openpouch's own hosting instead, run `openpouch deploy . --app <name>` — the " +
            governedProviders.join("/") +
            " service keeps running untouched (your rollback anchor) until you retire it. Recipe: docs/WORKFLOWS.md §10.",
        );
      }
      migratingFrom = governedProviders;
    }
  ...
  // L12 preflight: without a key there is no ownership to update — stop locally
  // (usage-grade) before spending an upload the server would 403 anyway.
  if (appName !== undefined && !authenticated) {
    return errorResult(
      EXIT.USAGE,
      "usage",
      "named apps (a stable URL across redeploys) need an account",
      "Sign up first (`openpouch signup --email you@example.com`), save your API key, then rerun the same deploy — or drop --app for an anonymous one-off preview.",
    );
  }
  ...
  try {
    const result = await adapter.instantDeploy(tarball, {
      ...
      ...(envOpts?.volume ? { volume: true } : {}),
      ...(envOpts?.alwaysOn ? { alwaysOn: true } : {}),
      ...(appName !== undefined ? { app: appName } : {}),
    });
    // Version skew (L12): an older box ignores the app header and mints a fresh
    // URL — surface that honestly instead of implying stability.
    const appNotSupportedByServer = appName !== undefined && result.app === undefined;
    const isNamedApp = result.app !== undefined;
    ...
    // Ownership comes from the server (run-d 0.3.1+). An absent field means an
    // older server that still hands every deploy the anonymous claim story —
    // keep matching what that server actually does (version skew: the box rolls
    // before the npm publish, so this window is the safe direction).
    const owned = result.ownership === "account";
    // F-02 (Codex 2026-07-12): follow-up commands must be runnable in the
    // caller's shell. Under `npx openpouch …` no global `openpouch` binary
    // exists — npm marks that invocation style with npm_command=exec — so keep
    // the npx form the caller used; a real binary on PATH keeps the bare form.
    const bin = ctx.env["npm_command"] === "exec" ? "npx -y openpouch" : "openpouch";
    // Persist the claim token where it won't be committed or relayed (Fix B).
    // Account-owned deploys carry no claim link — nothing to persist (F-01).
    if (result.claimUrl !== undefined && result.claimToken !== undefined) {
    ...
    if (!live) {
      ...
      nextStep = `The app is still starting (${result.status}). Give it a few seconds, then confirm it's reachable with \`${bin} verify\` before sharing the URL.`;
    } else {
      ...
      if (!health.checked || health.healthy) {
        ...
        if (healthPath !== undefined && healthPath !== "/" && health.checked) {
          ...
          if (!pathProbe.passed) {
            ...
            nextStep = `\`/\` answers, but the health path \`${healthPath}\` ${what} — the app shell is up while its API isn't. Inspect \`${bin} logs\`, fix the backend, redeploy with \`${bin} deploy\`, then \`${bin} verify --health-path ${healthPath}\`. Don't share the URL until it's healthy.`;
          }
        }
      } else {
        healthStatus = "degraded";
        ...
        nextStep = `The container is live but \`/\` ${what}. Inspect \`${bin} logs\`, fix the app (e.g. build the frontend before \`start\`), redeploy with \`${bin} deploy\`, then \`${bin} verify\`. Don't share the URL until it's healthy.`;
      }
    }
    ...
    await appendDeployment(ctx.cwd, {
      ...
      // stored privately in .openpouch/claim.json instead (anonymous deploys only;
      // an account-owned deploy is already saved and has no claim link, F-01).
      notes: isNamedApp
        ? `named app "${result.app}" — every \`deploy --app ${result.app}\` updates it behind this SAME URL (health-checked before the swap); stays live while used`
        : owned
          ? `account-owned instant deploy — stays live while used; expiresAt rolls forward on each request`
          : `instant preview — expires ${result.expiresAt}; private save link stored in .openpouch/claim.json`,
    });
    ...
    const baseSummary = instantDeploySummary({
      ...
      ...(owned ? { ownership: "account" as const } : {}),
      ...(isNamedApp ? { app: { name: result.app!, updated: wasUpdated } } : {}),
    });
    ...
    return summarized(
      EXIT.OK,
      {
        ...
        authenticated,
        // Ownership + expiry semantics, straight from the server (0.3.1+): agents
        // read these instead of inferring the retention story (Codex F-01).
        ...(result.ownership !== undefined ? { ownership: result.ownership } : {}),
        ...(result.expiryPolicy !== undefined ? { expiryPolicy: result.expiryPolicy } : {}),
        // L12: explicit machine truth about the URL's lifetime across redeploys —
        // "stable" only for a named app (same slug on every update); everything
        // else mints a fresh URL per deploy (the documented instant-lane default).
        urlStability: isNamedApp ? "stable" : "fresh-per-deploy",
        ...(isNamedApp ? { app: result.app, updated: wasUpdated } : {}),
        ...(appNotSupportedByServer ? { appNotSupportedByServer: true } : {}),
        // L13/G3: this deploy is a MIGRATION move — the app now runs on openpouch
        // hosting; the listed old-host services keep running untouched (rollback
        // anchor) until the human retires them. Never tell them a migration
        // happened elsewhere — THIS is the migration.
        ...(migratingFrom.length > 0 ? { migratingFrom } : {}),
        // Field-named hand-off (user feedback #1, 2026-07-15/16): free-running
        // agents trimmed the summary and the human never learned the expiry/keep
        // story — this travels under an imperative name they can't misread.
        // Only on a healthy deploy: a broken URL is not handed over (R9).
        ...(healthStatus === "healthy"
          ? {
              tellYourUser: tellYourUser({
                url: result.url,
                expiresAt: result.expiresAt,
                ...(owned ? { ownership: "account" as const } : {}),
                ...(isNamedApp ? { app: { name: result.app!, updated: wasUpdated } } : {}),
        ...
        // Account-owned deploys have NO claim fields at all — there is nothing to
        // claim on an app that's already saved to the account (Codex F-01).
        ...(owned
          ? {}
          : {
        ...
        nextCommands: {
          verify: `${bin} verify --json${healthPath !== undefined && healthPath !== "/" ? ` --health-path ${healthPath}` : ""}`,
          logs: `${bin} logs --json --limit 120`,
          inspect: `${bin} inspect --json`,
        },
        ...(storesData
        ...
        deployment: {
          id: result.slug,
          url: result.url,
          ...(redactSecrets || owned ? {} : { claimUrl: result.claimUrl }),
          expiresAt: result.expiresAt,
          status: result.status,
          kind,
          ...(isNamedApp ? { app: result.app } : {}),
      ...
      [
        `openpouch deploy ✓ ${hint} → ${
          isNamedApp
            ? `named app "${result.app}" ${wasUpdated ? "UPDATED behind its stable URL" : "created (this URL now stays stable across redeploys)"}`
            : owned
              ? "account-owned app"
              : "instant preview"
        }${kind === "dynamic" ? " (dynamic app — running in a container)" : ""}`,
        statusLine,
        ...noteLines,
        ...(appNotSupportedByServer
          ? [`  note:    the server ignored --app (older openpouch-run) — this deploy got a FRESH URL and will not update in place.`]
          : []),
        ...(migratingFrom.length > 0
          ? [`  migrate: the app now runs ON openpouch hosting; your ${migratingFrom.join("/")} service keeps running untouched (rollback anchor) until you retire it — data moves with \`openpouch data push\` (docs/WORKFLOWS.md §10).`]
          : []),
        ...
        owned
          ? `  expires: ${result.expiresAt} (account-owned — rolls forward while the app is used)`
