...
interface EnvironmentReport {
  ...
  /** `expiresAt` (instant lane): live server truth at read time — on a rolling
   *  account-owned record this is the CURRENT deadline (A-6, machine-readable
   *  persistence promise on the resume surface). */
  liveDeploy?: { id: string; status: string; commit?: string; commitMessage?: string; at?: string; expiresAt?: string };
  /**
...
export async function inspectCommand(ctx: CliContext): Promise<CommandResult> {
  ...
  for (const [name, report] of Object.entries(environments)) {
    ...
    if (report.liveDeploy !== undefined) {
      ...
      human.push(
        `    live: ${d.commit?.slice(0, 10) ?? d.id} ${d.commitMessage !== undefined ? `"${d.commitMessage}"` : ""} [${d.status}]${d.at !== undefined ? ` ${d.at}` : ""}${d.expiresAt !== undefined ? ` | expires ${d.expiresAt}` : ""}`,
      );
    }
