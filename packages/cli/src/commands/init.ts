...
export async function initCommand(
  ...
  if (existing.status !== "missing" && !flags.force) {
    ...
    return summarized(
      ...
      initSummary({ name: existingName, alreadyInitialized: true, agentsMd }),
      [
  ...
  if (apiKey === undefined) {
    ...
    try {
      ...
      if (matched !== undefined) {
        ...
        // Lane truth (field report 2026-07-16): an agent read the auto-match as
        // "openpouch = governance layer over the old host" and told its human a
        // migration had happened. Auto-match is the BYO lane — the app keeps
        // running on the matched provider. Say so, and name the own-hosting path.
        hints.push(
          "mapped your EXISTING render service (BYO governance lane) — the app keeps running on render; " +
            "openpouch governs deploys there. To MOVE the app onto openpouch's own hosting instead, run " +
            "`openpouch deploy` (migration recipe: docs/WORKFLOWS.md §10)",
        );
      } else {
