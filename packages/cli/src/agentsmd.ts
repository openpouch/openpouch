...
export function renderAgentsSection(projectName: string): string {
  return [
    ...
    "- **Keep a preview (claim-to-keep):** anonymous previews vanish after ~72h. Claiming extends one to 7 days — and saving it with a free account keeps it live as long as it's used (it only expires after ~90 days without a single request, with a warning mail first; any visit resets the clock). Hand your human the `claimUrl` **privately** — it carries a save token that works like a password, so pass it directly (never paste it into a shared report, PR, or chat). It's kept locally in `.openpouch/claim.json` (gitignored), never in `deploy.evidence.json` or the `summary`. The shareable public address is `url`.",
    "- **More capacity / persistence (paid, human pays):** `openpouch upgrade --plan <name>` mints a checkout URL — hand it to your human; payment happens in the browser at the Merchant of Record, never through an agent. The tier updates automatically after payment (`openpouch whoami` to verify). Paid tiers add capacity, always-on apps (`deploy --always-on`) and a persistent `/data` volume (`deploy --volume`).",
    "- **Migrate an app FROM another host (Render/Vercel/…) ONTO openpouch — fully agent-runnable:** (1) `openpouch deploy . --app <name>` puts the app on openpouch's own hosting with ONE stable URL across all future updates (add `--volume` if it stores data on disk, `--var K=V` for its env). (2) Move stored data: export it from the old host with your own means (its shell/dashboard, or a temporary export route you add to the app — you own the code), then `openpouch data push <name> <dir>` — this REPLACES the app's /data and restarts it health-gated. (3) `openpouch verify`, then tell your human: the NEW URL, and that the OLD host keeps running untouched as the rollback anchor until THEY confirm the cutover and retire it. NEVER call a governance mapping (init auto-match) a migration — a migration means the app RUNS on openpouch.",
