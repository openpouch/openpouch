...
describe("openpouch init", () => {
  it("detects the project, writes manifest + policy, maps the matching service", async () => {
    ...
    // Lane truth on auto-match (field report 2026-07-16): the hints must say the
    // app KEEPS RUNNING on the matched host (BYO lane) and point at the
    // own-hosting migration path, so an agent never sells governance as a migration.
    const hints = (JSON.parse(result.output) as { hints: string[] }).hints;
    expect(hints.some((h) => h.includes("BYO governance lane"))).toBe(true);
    expect(hints.some((h) => h.includes("WORKFLOWS.md"))).toBe(true);
...
describe("openpouch init — test-only sources are not runtime requirements (L4)", () => {
  it("ignores env vars and data signals that only tests reference", async () => {
    const dir = await mkdtemp(join(tmpdir(), "openpouch-testsrc-"));
    await writeFile(join(dir, "package.json"), JSON.stringify({ name: "t", dependencies: { express: "^4" }, scripts: { start: "node src/server.js" } }));
    await mkdir(join(dir, "src"), { recursive: true });
    await writeFile(join(dir, "src", "server.js"), "const k = process.env.API_KEY;");
    await mkdir(join(dir, "test"), { recursive: true });
    // References API_KEY too: a var the app itself reads must survive the exclusion.
    await writeFile(join(dir, "test", "server.test.js"), "process.env.API_KEY; process.env.TEST_TOKEN; require('fs').writeFileSync('x','y');");
    await writeFile(join(dir, "test-webhook.js"), "process.env.WEBHOOK_TEST_SECRET;");
    await writeFile(join(dir, "vitest.config.ts"), "process.env.CI_ONLY_VAR;");
    const json = JSON.parse((await run(["init", "--json"], ctx(dir))).output) as {
      detected: { envVars: string[]; dataPersistence: { storesData: boolean } };
    };
    expect(json.detected.envVars).toEqual(["API_KEY"]);
    expect(json.detected.dataPersistence.storesData).toBe(false);
  });

  it("skips __tests__ dirs and .spec files by convention", async () => {
    const dir = await mkdtemp(join(tmpdir(), "openpouch-testsrc2-"));
    await writeFile(join(dir, "package.json"), JSON.stringify({ name: "t2", dependencies: { express: "^4" }, scripts: { start: "node index.js" } }));
    await writeFile(join(dir, "index.js"), "process.env.DATABASE_URL;");
    await mkdir(join(dir, "__tests__"), { recursive: true });
    await writeFile(join(dir, "__tests__", "db.js"), "process.env.FAKE_DB_URL;");
    await writeFile(join(dir, "api.spec.ts"), "process.env.SPEC_ONLY;");
    const json = JSON.parse((await run(["init", "--json"], ctx(dir))).output) as { detected: { envVars: string[] } };
    expect(json.detected.envVars).toEqual(["DATABASE_URL"]);
  });
});
...
describe("command-specific help (OpenClaw 2026-07-04)", () => {
  ...
  it("every registered command has a help page", async () => {
    for (const cmd of ["deploy", "list", "delete", "data", "signup", "activate", "whoami", "init", "inspect", "plan", "preview", "prod", "approve", "verify", "logs", "rollback", "feedback"]) {
