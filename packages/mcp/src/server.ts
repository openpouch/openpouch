...
export function createOpenpouchServer(options: ServerOptions = {}): McpServer {
    ...
    "openpouch_deploy",
    {
      ...
      inputSchema: {
        ...
        volume: z
          .boolean()
          .optional()
          .describe("Paid tiers: mount the account's persistent /data volume (keyed to account+project — survives redeploys). Dynamic apps only; denied with an honest fix on free tiers."),
        alwaysOn: z
          .boolean()
          .optional()
          .describe("Paid tiers: keep the container running (no scale-to-zero cold start). Capped per tier; denied with an honest fix when unavailable."),
      },
    },
    async ({ cwd, dir, env, healthPath, redactSecrets, volume, alwaysOn }) =>
      runTool(() =>
  ...
  server.registerTool(
    "openpouch_upgrade",
    {
      ...
      inputSchema: { plan: z.string().describe("Paid tier to buy, e.g. starter | pro | scale | business") },
    },
    async ({ plan }) => runTool(() => upgradeCommand(makeCtx(), plan)),
  );
  ...
  server.registerTool(
    "openpouch_data_push",
    {
      ...
      inputSchema: {
        app: z.string().describe("The named app (created with deploy --app <name> --volume)"),
        dir: z.string().describe("Local directory whose contents REPLACE the app's /data"),
      },
    },
    async ({ app, dir }) => runTool(() => dataCommand(makeCtx(), "push", app, dir)),
  );
  ...
  server.registerTool(
    "openpouch_data_pull",
    {
      ...
      inputSchema: {
        app: z.string().describe("The named app whose /data to back up"),
        dest: z.string().optional().describe("Local file path for the backup (default: <app>-data.tgz)"),
      },
      // No readOnlyHint: it WRITES the backup archive to the local filesystem.
    },
    async ({ app, dest }) => runTool(() => dataCommand(makeCtx(), "pull", app, dest)),
  );
  ...
  server.registerTool(
    "openpouch_data_ls",
    {
      ...
      inputSchema: { app: z.string().describe("The named app whose /data to list") },
      annotations: readOnly,
    },
    async ({ app }) => runTool(() => dataCommand(makeCtx(), "ls", app)),
  );
