...
describe("openpouch MCP server (harness-neutral, stdio-compatible)", () => {
  it("exposes EXACTLY the 17-tool set — and deliberately NO approve tool", async () => {
    ...
    // the CI dist-smoke and the docs (MCP.md; the live llms.txt stays at 14
    // until this release actually ships — it describes npm latest).
  ...
  it("marks the read-only tools with readOnlyHint (agent permission-UX, R9)", async () => {
    ...
    const readOnly = new Set(["openpouch_inspect", "openpouch_plan", "openpouch_logs", "openpouch_whoami", "openpouch_list", "openpouch_list_approvals", "openpouch_data_ls"]);
