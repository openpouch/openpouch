...
| `deployments[].expiresAt` | string (ISO-8601 UTC) | no | — | instant-preview expiry; **0.3.0+ (D25):** ROLLING for account-owned records (last activity + inactivity window); **0.3.1:** account-owned records START on the rolling window at create — the 201 no longer shows the 72 h TTL first (Codex F-01) |
| `deployments[].lastRequestAt` | string (ISO-8601 UTC) | yes | — | **0.3.0+ (D25):** last observed inbound request (activity floor for usage-based persistence; write-damped to ~1/h) |
| `deployments[].inactivityWarnedAt` | string (ISO-8601 UTC) | yes | — | **0.3.0+ (D25):** set when the T-7 inactivity warning mail went out; cleared by new activity |
| `deployments[].volumeProject` | string | yes | — | **0.3.0+ (L10):** key of the account's persistent /data volume (`volumes/<accountId>/<project>` on the box) |
| `deployments[].alwaysOn` | boolean | yes | — | **0.3.0+:** never idle-stopped; restarted proactively on boot (paid tiers, capped by `maxAlwaysOn`) |
...
## counters.json (M-1, 0.3.0+)

Aggregate daily tallies in the run-d data dir — **whole-number counts only, never per-user/account/IP data** (privacy guardrail for the weekly usage monitor). Shape: `{ "<YYYY-MM-DD UTC>": { "deploys": {"static": n, "dynamic": n, "anon": n, "keyed": n}, "claims": {"token": n, "account": n}, "quota429": {"<reason>": n}, "signups": n, "activations": n, "checkouts": n, "upgrades": n, "cancels": n, "expiries": {"anon": n, "claimed7d": n, "inactive90d": n}, "inactivityWarnMails": n, "wakes": n } }`. Written atomically (temp + rename); buckets older than ~400 days are pruned on rollover. Increment sites: REBUILD Step 49.
