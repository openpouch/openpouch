...
|---|---|---|
| [`deploy.schema.json`](schemas/deploy.schema.json) | `openpouch deploy [dir] --json` · MCP `openpouch_deploy` | success shape; claim fields exist only on ANONYMOUS deploys (there: exactly one of `claimUrl` / `claimUrlRedacted`); `ownership` + `expiryPolicy` since 0.3.1 — account-owned deploys carry no claim fields and `claimUrlIsPrivate` became conditional (was required ≤0.3.0) |
...
| [`upgrade.schema.json`](schemas/upgrade.schema.json) | `openpouch upgrade --json` · MCP `openpouch_upgrade` (0.3.0+) | payment is human-only: relay `checkoutUrl`, never pay |
