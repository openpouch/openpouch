...
**Derived from:** `packages/mcp/src/server.ts` · **As of:** 2026-07-18 — **17 tools** (the count is the stable contract; the current package version is whatever `npm view @openpouch/mcp version` says — this doc deliberately pins no version number) · Transport: **stdio** (open MCP standard — works with any MCP client; harness neutrality is design rule D13).
...
4. All tools accept optional `cwd` (project directory; default: server cwd) **EXCEPT `openpouch_whoami`/`openpouch_upgrade`/`openpouch_list`/`openpouch_delete`** — those are account-scoped or slug-only and take no project dir.
...
| `openpouch_upgrade` | `plan: string` | `upgradeCommand` | mints a hosted-checkout URL for a paid tier — **payment itself is human-only**: the agent hands `checkoutUrl` to its human, who pays at the Merchant of Record; no pay tool, no card data on any agent surface; unknown plan → error names `availablePlans`; needs an account key (added in 0.3.0) |
...
| `openpouch_data_push` | `app: string`, `dir: string` | `dataCommand(push)` | REPLACES the named app's persistent `/data` with the directory's contents (L13 migration/restore primitive) — server stops the app for the write, restarts health-gated (a SQLite file is never swapped under a running process); owner-only; file CONTENTS never in logs/results (counts/sizes only) |
| `openpouch_data_pull` | `app: string`, `dest?: string` | `dataCommand(pull)` | downloads `/data` as a gzipped tar (the BACKUP primitive — no lock-in); owner-only; deliberately **no** `readOnlyHint` — it writes the local archive file |
| `openpouch_data_ls` | `app: string` | `dataCommand(ls)` | read-only inventory of `/data` — names/sizes/timestamps only, never contents; the verify step after a push/migration |

**Account signup/activation are deliberately CLI + web only** (they involve a human email or browser step, SSOT D16) — not MCP tools. `openpouch_whoami`, `openpouch_upgrade`, `openpouch_list`, and `openpouch_delete` are the account-surface tools exposed to agents (whoami + list are read-only; delete is self-service quota management on an ephemeral preview — not a governed-production action, so it needs no approval; upgrade only mints a checkout URL — the payment itself stays a human act, D13). There is still no approve tool (D13).
...
Expected (POSIX shell; the `sleep` keeps stdin open long enough for the reply): **17 tool names, and `openpouch_approve` is NOT among them** — approving stays human-only in every harness. Count them with `… | wc -l` → `17`. This is the same probe our own release gate runs against the built bundle before every publish; agent test suites should use it instead of marking MCP checks as SKIP when the harness can't load servers dynamically (tool EXECUTION parity still needs a real MCP client — only that part remains harness-dependent).

**Copy-safe one-liner** — the exact same probe with no line continuations. A copy/fetch step that drops the leading `(` of the multi-line form fails with `syntax error near unexpected token ')'` (seen in a real agent field run, 2026-07-12); this form survives any single-line copy:

```sh
( printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"smoke","version":"0"}}}' '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}'; sleep 2 ) | npx -y @openpouch/mcp 2>/dev/null | grep -o '"name":"openpouch_[a-z_]*"' | sort -u
```
...
`packages/mcp/test/mcp.test.ts` via the SDK's `InMemoryTransport` (client ↔ server, no stdio needed): toolset completeness incl. the no-approve-tool guarantee, inspect round-trip, the plain-language summary surfaced as a leading relay block **and** inside `json.summary` (R3 / D13 parity), production approval gate (`isError` + request id, visible in `openpouch_list_approvals`), logs with limit, and `openpouch_whoami` (anonymous path). The leading-relay-block + 14-tools/no-approve invariants were also smoke-checked over real stdio against the built `bin/openpouch-mcp.js`.

## Added in 0.3.0

- **`openpouch_upgrade`** — mints a hosted-checkout URL for a paid tier (`plan` input). Payment is deliberately a HUMAN step: the agent hands the `checkoutUrl` to its human, who pays in the browser at the Merchant of Record; there is no pay tool and no card data ever reaches an agent surface. The account tier updates via signed webhook; verify with `openpouch_whoami`. Added in 0.3.0.
