...
- **Restricted-egress sandboxes** (package-manager-only allowlists — npm/GitHub/PyPI reachable, everything else blocked): installing the CLI **works** (npm is allowed), but the deploy call itself needs **HTTPS egress to `openpouch.sh`** and fails as a classified `network` error (category, host, cause, and the `curl https://openpouch.sh/healthz` self-test in the `fix`). Resolution: allow `openpouch.sh` (API + deployed apps) and `openpouch.dev` (docs) in the sandbox's network policy. **In consumer agent apps that allowlist is often USER-configurable (environment/network settings) — name the two hostnames to your user; it's a one-time step they can do themselves, not an ops ticket.** (Second field report, 2026-07-16: a mobile coding-agent sandbox allowed only GitHub/npm/Anthropic; the user extended the policy the moment the agent named the hosts. Vercel's API was equally blocked there — an environment property, not an openpouch gap; the agent's own diagnosis "policy refusal, not an openpouch problem" was exactly right.) Otherwise deploy from an environment with general HTTPS egress. A package-manager allowlist alone is NOT enough.
