...
| `pendingClaim` | `{deploymentId, claimToken}`? | **0.3.0+ (D25):** claim-page signup — bind this deployment when the email verifies (handed back exactly once). |
| `pendingBind` | `{claim, hashedToken, expiresAt}`? | **0.3.0+ (D25):** mail-confirmed bind for an EXISTING account (one-time token, 24h, `sha256` stored). |
...
**Since 0.3.0 (D25):** `POST /api/accounts` accepts an optional `claim: {id, token}` (the deployment's claim token doubles as the ownership proof, constant-time checked; invalid → 400 `invalid-claim`). Fresh email → the pending account stores it and the verify step ties the deployment to the new account (`boundDeployment` in the JSON answer). An email that already owns an ACTIVE account gets a one-time bind-confirm mail instead (`GET /api/accounts/bind?account&token`, 24h, single-use) — both branches return the identical `201` body, so the endpoint stays useless as an account-enumeration oracle. Binding starts the usage-based persistence clock (see INSTANT-LANE §Lifecycle).
