...
## Billing routes (0.3.0+; dormant without a configured provider)

- `POST /api/billing/checkout` (Bearer key): `{plan}` → `201 {checkoutUrl, plan, founderPricing, summary}`; unknown plan → `400` with `availablePlans`; billing disabled → `503`. Founder cohort: accounts created before `OPENPOUCH_RUN_BILLING_GA_DATE` check out at launch prices.
- `POST /api/billing/webhook` (public): Paddle-signed (`Paddle-Signature`, HMAC-SHA256 over `ts:rawBody`, small replay window). Idempotent by event id; out-of-order events are ignored via `lastEventAt`; `subscription.canceled` downgrades to `free` NON-destructively; `past_due` leaves the tier to the MoR's dunning. Invalid signature → `403` without detail.
